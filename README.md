...
- **실시간 경영**: 시간 흐름에 따른 동적 게임 진행 (1x/2x/3x 속도)
- **경제 시뮬레이션**: 현실적인 원가 계산 및 수익성 분석
- **전략적 의사결정**: 자원 배분, 투자, 대출 관리
- **이벤트 시스템**: 랜덤 이벤트 및 선택지 (정부 지원금, 파업, 설비 투자 등)
- **기술 연구 개발**: 16가지 연구 기술로 회사 경쟁력 향상
  - 생산 효율, 품질 관리, 원가 절감, 건조 속도, 안전 관리, 자동화
- **경쟁사 AI**: 8개 글로벌 조선소와 실시간 경쟁
  - Hyundai Heavy, Samsung Heavy, Daewoo, CSSC, Fincantieri 등
  - 각 경쟁사마다 고유한 AI 특성 (공격성, 연구 집중도, 확장 속도)
  - 월별 성장 시뮬레이션 및 시장 점유율 경쟁
- **난이도 시스템**: 4단계 난이도로 다양한 도전 제공
  - 쉬움, 보통, 어려움, 전문가
  - 난이도별 초기 자금, 부채, 도크, 이자율 차별화
- **업적 시스템**: 30개 업적으로 목표 달성 추적
  - 7개 카테고리 (이정표, 생산, 재무, 평판, 연구, 시장, 특수)
  - 5개 희귀도 (일반 → 전설)
  - 업적 달성 시 현금 및 평판 보상
...
│   └── index.ts        # 게임 타입들
├── utils/              # 유틸리티 함수
│   └── saveManager.ts  # 저장/불러오기 관리
├── engine/             # 게임 엔진 로직
│   ├── bidGenerator.ts            # 입찰 생성
│   ├── financeCalculator.ts       # 재무 계산
│   ├── eventGenerator.ts          # 이벤트 생성
│   └── eventGeneratorWithEffects.ts # 효과가 있는 이벤트
├── data/               # 게임 데이터
│   ├── ships.ts        # 선박 스펙
│   ├── customers.ts    # 고객사 데이터
│   └── research.ts     # 연구 기술 데이터
├── App.tsx             # 메인 앱
└── main.tsx            # 엔트리 포인트
\`\`\`
...
### ✅ Phase 3: 게임 확장 (완료)
- [x] 이벤트 시스템 완성 (시장, 재무, 생산, 계약)
- [x] 랜덤 이벤트 발생 및 선택지 (정부 지원금, 설비 투자, 파업 등 7가지)
- [x] 이벤트 알림 모달 UI
- [x] 기술 연구 개발 시스템 (16가지 연구, 6개 카테고리)
- [x] 연구 진행도 자동 업데이트
- [x] 연구 완료 시 효과 자동 적용
- [x] 경쟁사 AI (8개 글로벌 조선소)
- [x] 글로벌 시장 분석 UI
...
### ✅ Phase 4: 고급 기능 (완료)
- [x] 저장/불러오기
- [x] 난이도 설정 (4단계: 쉬움/보통/어려움/전문가)
- [x] 업적 시스템 (30개 업적, 7개 카테고리, 5개 희귀도)
- [x] 통계 및 분석 대시보드 (Recharts 기반 6가지 차트)

### ✅ Phase 5: 게임 완성도 (완료)
- [x] 튜토리얼 시스템 (16단계 대화형 가이드)
- [x] 설정 메뉴 (자동저장, 알림, 사운드 설정)
- [x] 게임 종료 조건 (4가지 승리/3가지 파산)
- [x] 다중 저장 슬롯 (5개 슬롯, 저장/불러오기/삭제)
- [ ] 클라우드 저장 (Firebase)
- [ ] 멀티플레이어 모드
