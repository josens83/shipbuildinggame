...
import type { TutorialDialog } from '../../types/dialog';

interface TutorialDialogProps {
  dialog: TutorialDialog;
}

export default function TutorialDialogComponent({ dialog }: TutorialDialogProps) {
  const { nextTutorialStep, prevTutorialStep, skipTutorial } = useDialogStore();

  const currentStep = dialog.step;
  const totalSteps = dialog.totalSteps;
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-scale-in">
        ...
        <div className="border-t border-gray-200 p-4 flex items-center justify-between bg-gray-50">
          ...
          <div className="flex items-center gap-2">
            {dialog.canGoBack !== false && !isFirstStep && (
              <button
                onClick={handlePrev}
